...
1.  **提交格式 (Type)**：
    *   如果不配置，默认可能只是简单的描述。
    *   若需生成符合 Conventional Commits 规范（如 `feat: add new feature`）的消息，**必须**执行：
        ```bash
...
> 💡 **提示**：所有 `flow ai` 开头的配置命令实际上都是在调用 `aicommits config`。更多高级用法请查阅 [aicommits 官方文档](https://github.com/Nutlope/aicommits)。

### 阶段 1：特性同步 (Feature Sync Pro)

**目标**：将个人开发分支（如 `feat/user-task-01`）以**线性原子提交**的方式合并到公共特性分支（如 `feat/1.0.0`）。
...
*   [aicommits](https://github.com/Nutlope/aicommits): AI 自动生成提交信息。

...
### 零阶段：主干同步 (Master Sync)

在开发过程中，如果 Master 分支有重要更新（如核心库升级、公共配置变更），开发者需要将 Master 代码同步到当前分支。

> ⚠️ **注意**：请使用 `flow` -> `主干同步 (master-sync)` 功能，工具会根据你的分支类型自动选择最佳策略。

*   **个人独占分支 (Private)**：
    *   **策略**：`Rebase` (变基)。
    *   **原理**：将你的私有提交“嫁接”到 Master 最新节点之后。
    *   **优势**：保持当前分支历史线性整洁，如同你刚刚基于最新的 Master 切出分支一样。
    *   **注意**：同步后推送远程需要 `git push --force-with-lease`。

*   **公共协作分支 (Public)**：
    *   **策略**：`Merge` (合并)。
    *   **原理**：生成一个 Merge Commit 节点，保留真实的历史合并轨迹。
    *   **优势**：避免破坏其他协作成员的提交历史（公共分支严禁 Rebase）。
...
*   **Q: `git push -f` 会覆盖别人的代码吗？**
    *   A: 在**私有分支**上操作是安全的。但在公共分支（如 feat/1.0.0）上**严禁使用**强制推送。
